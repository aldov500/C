...
void ConfigurarTelnet(){
  ...
  TELNET.begin(115200);
  TELNET.println("\n"); delay(1000);
  TELNET.print("Iniciando servidor Telnet");
  ...
  TELNET.println("Iniciando Punto de Acceso");
  WiFi.mode(WIFI_AP);
  ...
  TELNET.print("Servidor Telnet iniciado! Conexión telnet por medio de ");
  TELNET.print(WiFi.localIP());
  TELNET.print(' ');
  TELNET.print(TelnetPuerto);
  TELNET.println(" para conexión.");
}
...
void RecibirTelnet(){
  ...
  for (aux = 0; aux < TelnetNumClientes; aux++) {
    if (TelnetWifiCliente[aux] && TelnetWifiCliente[aux].connected()) {
      ...
      if(EstaDesconectado = false){
          TELNET.println("Usuario desconectado.");
...
void ConectarTelnet() {
  ...
  for (int i = 0; i < TelnetNumClientes; i++) {
    if (TelnetWifiServer.hasClient()) {
      String RespuestaConexion = "Conexion de cliente exitosa.";
      ...
      if (!TelnetWifiCliente[i] || !TelnetWifiCliente[i].connected()) {
        ...
        Serial.print(RespuestaConexion);
        EscribirStringTelnet("Conectado a Telnet");
      }
