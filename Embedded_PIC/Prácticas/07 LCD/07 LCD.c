...
void main() 
{
 ...
 while(8)
 {
   Lcd_Out(1, 1, "  Practica 07");      //Mensaje
   Lcd_Out(2, 1, "C para Embebidos");   //        en reposo
   

   if (PORTA.RA0)                       //Muestra mensaje1
     {
      Lcd_cmd(_LCD_CLEAR);
      delay_ms(250);
      Lcd_Out(1, 1, TEXTO1);            //Muestra nombre y
      Delay_ms(250);
      Lcd_Out(2, 1, TEXTO2);            //código en el display
      delay_ms(1000);
      
      for(i=0; i<17; i++)
      {
       Lcd_Cmd(_LCD_SHIFT_LEFT); //Mueve el texto a la izquiera
       delay_ms(150);
      }
      delay_ms(500);
   ...
   if (PORTA.RA1)                      //Animación
      {
       Lcd_cmd(_LCD_CLEAR);
       Lcd_Out(1, 5, "-PACMAN-");
       delay_ms(1000);
       
       for (i=0; i!=17; i++)          //Ciclo For para el movimiento
           {
             Lcd_cmd(_LCD_CLEAR);
             Phantom(2,i);           //Función de caracter personal "Fantasma"
             Lcd_Cmd(_LCD_SHIFT_RIGHT);
             delay_ms(200);

             PacMan1(2,i);           //Pac-Man boca abierta
             delay_ms(200);
             PacMan2(2,i);
             delay_ms(100);           //Pac-Man boca cerrada
             Lcd_Cmd(_LCD_SHIFT_RIGHT);
